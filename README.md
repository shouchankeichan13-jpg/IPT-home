...
<style>
  * { box-sizing: border-box; }
  ...
  .call-container { display: flex; flex-direction: column; align-items: center; padding-top: 80px; height: 100%; }
  .iphone-name { font-size: 34px; font-weight: 300; margin-bottom: 8px; }
  .iphone-timer { font-size: 18px; color: #d1d1d1; margin-bottom: 50px; }
  
  .icon-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 25px; width: 280px; }
  .icon-box { display: flex; flex-direction: column; align-items: center; }
  .circle-btn { width: 72px; height: 72px; border-radius: 50%; background: rgba(255,255,255,0.15); border: none; color: white; margin-bottom: 8px; font-size: 12px; }
...
<div id="dialPage">
  <font size="5"><b>IPT 電話 V16.0</b></font><br>
...
<div id="callPage">
  <div class="call-container">
    ...
    <div class="icon-grid">
      <div class="icon-box"><button class="circle-btn">消音</button><span class="icon-label">消音</span></div>
      <div class="icon-box"><button class="circle-btn">キーパッド</button><span class="icon-label">キーパッド</span></div>
      <div class="icon-box"><button class="circle-btn">スピーカー</button><span class="icon-label">スピーカー</span></div>
      <div class="icon-box"><button class="circle-btn">+</button><span class="icon-label">通話追加</span></div>
      <div class="icon-box"><button class="circle-btn">ビデオ</button><span class="icon-label">FaceTime</span></div>
      <div class="icon-box"><button class="circle-btn">連絡先</button><span class="icon-label">連絡先</span></div>
    </div>
...
<script>
  ...
  function playRBT(){
    ...
    c.frequency.value=400; m.frequency.value=16; mg.gain.value=15; // 音程を安定させた16Hz変調
  ...
  function handleMiddleClick(e){ if(e.button===1 && document.getElementById('callPage').style.display!=='block'){ add(keys[cur]); e.preventDefault(); } }
